...
@inline function applyInteractionFunction!(
    ...
    cell_cartesian_index = getPositionCartesianIndexFromCellLinkList(getPosition(p), particle_system.cell_link_list_)
    ...
    @inbounds for i_neighbour_cell in 1:length(cell.neighbour_cell_cartesian_grid_index_list_)
        ...
        for j in 1:length(neighbour_cell)
            ...
            @inbounds q = particle_system[q_index]
            rpq = getPosition(p) - getPosition(q) + neighbourRelativePositionDisplacement(cell, i_neighbour_cell)
            r = norm(rpq)
            if r > particle_system.cell_link_list_.reference_gap_
                continue
            end
            interactionFunction!(p, q, rpq, r; parameters...)
        end
    end
    return nothing
end
