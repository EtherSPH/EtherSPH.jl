...
class CruchagaPostProcess:
    ...
    def __init__(self, working_directory: str = f"example/{POST_NAME}", referece_gap: float = 0.004, key_word: str = "3d") -> None:
    ...
    def fastplot(self, step: int, scalars: str = "Velocity", point_size: str = 6) -> pv.Plotter:
        plotter: pv.Plotter = pv.Plotter()
        poly_data: pv.PolyData = self.readStep(step)
        plotter.add_mesh(poly_data, scalars=scalars, cmap="coolwarm", point_size=point_size, nan_opacity=0.1)
    ...
    def viewPlotStep(self, i: int, view_step: int, scalars: str = "Velocity", point_size: str = 6) -> None:
    ...
    def referencePlot(self) -> None:
        ...
        plt.plot(t, x, label="SPH - Cruchaga 3D", color="blue")
        ...
        plt.title("Cruchaga 3D - SPH vs Experimental 3D")
        plt.grid()
        plt.legend()
