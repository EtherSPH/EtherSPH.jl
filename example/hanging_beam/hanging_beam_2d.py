...
class HangingBeam2DPostProcess:
    ...
    def __init__(self, working_directory: str = f"example/{POST_NAME}", reference_gap: float = 0.01, key_word: str = "2d") -> None:
    ...
    def viewPlotStep(self, plot_step: int = 0, scalars: str = "Velocity", point_size: str = 8) -> None:
        plotter: pv.Plotter = pv.Plotter(off_screen=True)
        poly_data: pv.PolyData = self.readStep(plot_step)
        plotter.add_mesh(poly_data, scalars=scalars, cmap="jet", point_size=point_size)
        t, step = self.getTime(poly_data), self.getStep(poly_data)
        plotter.add_text(f"Time: {t:.4f} s  Step: {step}", font_size=15, position="upper_edge")
        plotter.add_mesh(poly_data.extract_points([self.monitor_point_index]), color="blue", point_size=10)
    ...
    def referencePlot(self) -> None:
        ...
        plt.savefig(os.path.join(self.working_directory, f"image/{POST_NAME}_2d_comparison.png"), bbox_inches="tight", dpi=300)
        pass
