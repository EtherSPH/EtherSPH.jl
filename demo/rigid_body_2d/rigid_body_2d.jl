...
@kwdef mutable struct RigidBody
...
@inline function moveRigidBody!(rigid_body::RigidBody)::Nothing
    for p in rigid_body.particles_
        rigid_body.force_vec_ .+= p.mass_ * p.dv_vec_
        rigid_body.torque_ += p.mass_ * cross(p.x_vec_ .- rigid_body.x_vec_, p.dv_vec_)
    end
    rigid_body.dv_vec_ .= rigid_body.force_vec_ / rigid_body.mass_ .+ g
    rigid_body.alpha_ = rigid_body.torque_ / rigid_body.inertia_
    Threads.@threads for p in rigid_body.particles_
        p.dv_vec_ .= 0.0
        direction_vec = Vector2D(-(p.x_vec_[2] - rigid_body.x_vec_[2]), (p.x_vec_[1] - rigid_body.x_vec_[1]))
        p.v_vec_ .= rigid_body.v_vec_ .+ rigid_body.omega_ * direction_vec .+ rigid_body.alpha_ * dt * direction_vec
        p.x_vec_ .+=
            rigid_body.v_vec_ * dt .+ rigid_body.omega_ * dt * direction_vec .+
            0.5 * rigid_body.alpha_ * dt * dt * direction_vec
    end
    rigid_body.x_vec_ .+= rigid_body.v_vec_ * dt .+ 0.5 * rigid_body.dv_vec_ * dt * dt
    rigid_body.v_vec_ .+= rigid_body.dv_vec_ * dt
    rigid_body.omega_ += rigid_body.alpha_ * dt
    rigid_body.force_vec_ .= 0.0
    rigid_body.torque_ = 0.0
    return nothing
end

@inline function correctRigidBodyCenter!(rigid_body::RigidBody)::Nothing
    sum_mass = 0.0
    rigid_body.x_vec_ .= 0.0
    for p in rigid_body.particles_
        sum_mass += p.mass_
        rigid_body.x_vec_ .+= p.mass_ * p.x_vec_
    end
    rigid_body.x_vec_ ./= sum_mass
    return nothing
end
...
rigid_body = RigidBody(
    x_vec_ = Vector2D(rigid_body_center_x, rigid_body_center_y),
    mass_ = rho_rigid_body * rigid_body_width * rigid_body_height,
    inertia_ = 1.0 / 12.0 *
               rho_rigid_body *
               rigid_body_width *
               rigid_body_height *
               (rigid_body_width^2 + rigid_body_height^2),
    particles_ = rigid_body_particles,
)
...
vtp_writer.file_name_ = "rigid_body_2d_"
vtp_writer.output_path_ = "demo/results/rigid_body_2d"
...
function main()::Nothing
    ...
    for step in ProgressBar(1:round(Int, total_time / dt))
        ...
        if step % round(Int, center_correct_dt / dt) == 0
            correctRigidBodyCenter!(rigid_body)
        end
